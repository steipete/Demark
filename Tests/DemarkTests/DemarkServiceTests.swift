...
struct DemarkServiceTests {
    ...
    func testNestedComplexStructures() async throws {
        let service = Demark()
        let markdown = try await service.convertToMarkdown(HTMLFixtures.nestedComplex)
        assertNestedComplexMarkdown(markdown)
    }
    ...
    func testConcurrentConversionsAdvanced() async throws {
        ...
        let results = await runConcurrentConversions(service: service, htmlSamples: htmlSamples)
        ...
        for (index, result) in results {
            ...
            assertConcurrentConversionResult(result, at: index)
        }
    }
    ...
    func testMarkdownOutputQuality() async throws {
        let service = Demark()
        let markdown = try await service.convertToMarkdown(HTMLFixtures.documentation)
        assertMarkdownQuality(markdown)
    }
        ...
        <section id="intro">
            ...
            <ol>
                ...
                <li>
                    ...
                    <blockquote>
                        <p>
                            External resources are available at
...
private func runConcurrentConversions(
    service: Demark,
    htmlSamples: [String]
) async -> [(Int, String)] {
...
private func assertConcurrentConversionResult(_ result: String, at index: Int) {
    switch index {
    ...
    default: #expect(Bool(false), "Unexpected index in results: \(index)")
    }
}

private func assertNestedComplexMarkdown(_ markdown: String) {
...
private func assertMarkdownQuality(_ markdown: String) {
